...
- Intel: SSH as cloudadmin (key-based)
- AMD: You may type serveradmin/password in the form, but the run will SSH as awsuser
       using a configured private key (serveradmin’s key). The typed SSH password is
       intentionally ignored for AMD to avoid password auth against awsuser.
...
PLAYBOOKS = {
    "intel": {
        ...
        "force_ssh_user": "cloudadmin",                 # always login as cloudadmin
        # "ssh_private_key": "/path/to/cloudadmin.pem", # optional if inventory already handles it
    },
    "amd": {
        "label": "AMD Health Check",
        "path": "/var/www/cgi-bin/amd-check.yml",
        "inventories": ["amd-inv"],
        "force_ssh_user": "awsuser",                    # login as awsuser on remote
        "ssh_private_key": "/var/lib/www-ansible/keys/serveradmin.pem",  # serveradmin’s key file
        "suggest_ssh_user": "serveradmin",              # shown in form; not used for SSH
    },
}
...
def render_form(msg: str = "", form: cgi.FieldStorage = None):
    ...
    # SSH user field value
    # If playbook suggests an SSH user for UX, prefill it, otherwise use typed or default.
...
<head>
  ...
  <script>
    ...
    function onPlaybookChanged() {{
      // When playbook changes, refresh to re-filter inventories and suggested user
      document.getElementById('action').value = 'refresh';
      document.getElementById('runnerForm').submit();
    }}
    ...
    <form id="runnerForm" method="post" action="">
      ...
      <div class="row">
        <div>
          ...
          <div class="muted">Intel ignores this (uses cloudadmin). AMD ignores for SSH (uses awsuser + key).</div>
        </div>
        <div>
      ...
      <div class="muted">Ignored for AMD SSH (key-based as awsuser). Used for Intel only if needed.</div>
...
def run_playbook(form: cgi.FieldStorage):
    ...
    # AMD: we SSH as awsuser with a key; do NOT try password auth for SSH
    if playbook_key == "amd":
        ssh_pass = ""
